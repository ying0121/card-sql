...
-- eso_t_c0012_kaiheiki.sql for store data migration from ESI_T_K102_CT_KAIHEIKI to eso_t_c0012_kaiheiki
-- 作成日: 2026-01-14
-- 作成者: 浦大輔
-- 対象DB：PostgreSQL

*---------------------------------------------------------*
CREATE OR REPLACE FUNCTION migrate_c0012_kaiheiki()
RETURNS TABLE(
    total_cnt   int,
    success_cnt int,
    skip_cnt    int,
...
DECLARE
    ...
    -- テーブル情報
    --------------------------------------------------
    v_fm_table_nm text := 'ESI_T_K102_CT_KAIHEIKI';        -- 移行元テーブル名
    v_to_table_nm text := 'eso_t_c0012_kaiheiki';          -- 移行先テーブル名

    --------------------------------------------------
    -- 更新情報(作成者～更新者)
    --------------------------------------------------
    v_create_date                   eso_t_c0012_kaiheiki.create_date%TYPE;      -- 作成日
    v_create_user                   eso_t_c0012_kaiheiki.create_user%TYPE;      -- 作成者
    v_record_date                   eso_t_c0012_kaiheiki.record_date%TYPE;      -- 更新日
    v_record_user                   eso_t_c0012_kaiheiki.record_user%TYPE;      -- 更新者

BEGIN
    RAISE LOG 'migration % start',
        v_to_table_nm;

    FOR rec_from IN SELECT * FROM ESI_T_K102_CT_KAIHEIKI LOOP
        v_total := v_total + 1;
        ...
        BEGIN
            ...
            INSERT INTO eso_t_c0012_kaiheiki (
                ...
                daisu_renban,                               --台数連番
                shinsetsu_flg,                              --新設フラグ
                genba_kakuninzumi_meibanto_flg,             --現場確認済（銘板等）フラグ
                shurui_cd,                                  --種類
                kyokusu,                                    --極数
                tkaku_a,                                    --定格電流
                tkaku_a_cd,                                 --定格電流単位
                tkaku_a_kakuninkonnan_flg,                  --定格電流確認困難フラグ
                fuse_kva,                                   --ヒューズ容量
                fuse_kva_cd,                                --ヒューズ容量単位
                fuse_kva_kakuninkonnan_flg,                 --ヒューズ容量確認困難フラグ
                kando_a,                                    --感度電流
                kando_a_cd,                                 --感度電流単位
                kando_a_kakuninkonnan_flg,                  --感度電流確認困難フラグ
                seizo_ym,                                   --製造年月
                seizo_nennomi_flg,                          --製造年のみ入力フラグ
                seizo_ym_kakuninkonnan_flg,                 --製造年月確認困難フラグ
                ikisaki,                                    --行先
                biko,                                       --備考
                ordr_jn,                                    --表示順
                create_date,                                --作成日
                ...
                rec_from.kyokusu,	                        -- 極数
                rec_from.tkaku_amp,	                        -- 定格電流（A）
                rec_from.tkaku_ampt_cd,	                    -- 定格電流単位
                0,		
                NULL,		
                '12',		
                0,	
                rec_from.kando_amp,	                        -- 感度電流
                rec_from.kando_ampt_cd,	                    -- 感度電流単位
                0,		
                rec_from.seizo_ym,	                        -- 製造年月
                rec_from.seizo_y_flg,	                    -- 製造年のみ入力フラグ
                0,		
                rec_from.ikisaki,	                        -- 行先
                NULL,		
                rec_from.ordr,	                            -- 表示順
