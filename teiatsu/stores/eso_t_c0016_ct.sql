...
-- eso_t_c0016_ct.sql for store data migration from ESI_T_K106_CT_CT to eso_t_c0016_ct
-- 作成日: 2025-01-06
-- 作成者: 浦大輔
-- 対象DB：PostgreSQL

*---------------------------------------------------------*
CREATE OR REPLACE FUNCTION migrate_c0016_ct()
RETURNS TABLE(
    total_cnt   int,
    success_cnt int,
    skip_cnt    int,
...
DECLARE
    ...
    -- テーブル情報
    --------------------------------------------------
    v_fm_table_nm text := 'ESI_T_K106_CT_CT';        -- 移行元テーブル名
    v_to_table_nm text := 'eso_t_c0016_ct';          -- 移行先テーブル名

    --------------------------------------------------
    -- 更新情報(作成者～更新者)
    --------------------------------------------------
    v_create_date                   eso_t_c0016_ct.create_date%TYPE;      -- 作成日
    v_create_user                   eso_t_c0016_ct.create_user%TYPE;      -- 作成者
    v_record_date                   eso_t_c0016_ct.record_date%TYPE;      -- 更新日
    v_record_user                   eso_t_c0016_ct.record_user%TYPE;      -- 更新者

BEGIN
    RAISE LOG 'migration % start',
        v_to_table_nm;

    FOR rec_from IN SELECT * FROM ESI_T_K106_CT_CT LOOP
        v_total := v_total + 1;
        ...
        BEGIN
            ...
            INSERT INTO eso_t_c0016_ct (
                ...
                tkaku_v_cd,                                 -- 定格電圧単位
                tkaku_1ji_a,                                -- 定格一次電流
                tkaku_1ji_a_cd,                             -- 定格一次電流単位
                tkaku_2ji_a,                                -- 定格二次電流
                tkaku_2ji_a_cd,                             -- 定格二次電流単位
                kantsu_su,                                  -- 巻数
                biko,                                       -- 備考
                ordr_jn,                                    -- 表示順
                create_date,                                -- 作成日
                ...
                rec_from.tkaku_vlt,	                        -- 定格電圧（V）
                rec_from.tkaku_vltt_cd,	                    -- 定格電圧単位
                rec_from.tkaku_amp1,	                    -- 定格１次電流（A）
                rec_from.tkaku_amp1t_cd,	                -- 定格１次電流単位
                rec_from.tkaku_amp2,	                    -- 定格２次電流（A）
                rec_from.tkaku_amp2t_cd,	                -- 定格２次電流単位
                rec_from.kantsu_cnt,	                    -- 貫通数
                NULL,
                rec_from.ordr,	                            -- 表示順
