...
-- eso_t_c0013_cable.sql for store data migration from ESI_T_K103_CT_CABLE to eso_t_c0013_cable
-- 作成日: 2026-01-14
-- 作成者: 浦大輔
-- 対象DB：PostgreSQL

*---------------------------------------------------------*
CREATE OR REPLACE FUNCTION migrate_c0013_cable()
RETURNS TABLE(
    total_cnt   int,
    success_cnt int,
    skip_cnt    int,
...
DECLARE
    ...
    -- テーブル情報
    --------------------------------------------------
    v_fm_table_nm text := 'ESI_T_K103_CT_CABLE';        -- 移行元テーブル名
    v_to_table_nm text := 'eso_t_c0013_cable';          -- 移行先テーブル名

    --------------------------------------------------
    -- 更新情報(作成者～更新者)
    --------------------------------------------------
    v_create_date                   eso_t_c0013_cable.create_date%TYPE;      -- 作成日
    v_create_user                   eso_t_c0013_cable.create_user%TYPE;      -- 作成者
    v_record_date                   eso_t_c0013_cable.record_date%TYPE;      -- 更新日
    v_record_user                   eso_t_c0013_cable.record_user%TYPE;      -- 更新者

BEGIN
    RAISE LOG 'migration % start',
        v_to_table_nm;

    FOR rec_from IN SELECT * FROM ESI_T_K103_CT_CABLE LOOP
        v_total := v_total + 1;
