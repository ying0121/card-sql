
-- 2025-12-18_example_views_usage.sql
-- 目的：テーブルクエリの使用例とテストクエリ
-- 作成日: 2025-12-18
-- 作成者: 浦大輔
-- 対象DB：PostgreSQL
--
-- 注意：このファイルは実際のテーブルを使用したクエリの例です。
-- 論理削除されていないレコードのみを取得するクエリです。

-- ============================================
-- テーブルクエリの使用例
-- ============================================

-- 1. 計器類テーブルの使用例
-- 論理削除されていないレコードのみを取得
BEGIN;
CREATE OR REPLACE VIEW v_eso_t_c0011_keiki AS
SELECT *
FROM eso_t_c0011_keiki
WHERE shinki_koushin_sakujo_flg IS DISTINCT FROM 2; -- 削除された行を除外
COMMIT;

-- 2. 低圧開閉器テーブルの使用例
BEGIN;
CREATE OR REPLACE VIEW v_eso_t_c0012_kaiheiki AS
SELECT *
FROM eso_t_c0012_kaiheiki
WHERE shinki_koushin_sakujo_flg IS DISTINCT FROM 2; -- 削除された行を除外
COMMIT;

-- 3. 低圧ケーブルテーブルの使用例
BEGIN;
CREATE OR REPLACE VIEW v_eso_t_c0013_cable AS
SELECT *
FROM eso_t_c0013_cable
WHERE shinki_koushin_sakujo_flg IS DISTINCT FROM 2; -- 削除された行を除外
COMMIT;

-- ============================================
-- テーブルの統計情報取得（2025-12-18時点）
-- ============================================

-- 各テーブルのレコード数を確認（論理削除除外）
-- 注意：この日付までに作成されたテーブルのみ（c0011-c0013）
BEGIN;
SELECT 'eso_t_c0011_keiki' AS table_name, COUNT(*) AS record_count
FROM eso_t_c0011_keiki
WHERE shinki_koushin_sakujo_flg IS DISTINCT FROM 2
UNION ALL
SELECT 'eso_t_c0012_kaiheiki', COUNT(*)
FROM eso_t_c0012_kaiheiki
WHERE shinki_koushin_sakujo_flg IS DISTINCT FROM 2
UNION ALL
SELECT 'eso_t_c0013_cable', COUNT(*)
FROM eso_t_c0013_cable
WHERE shinki_koushin_sakujo_flg IS DISTINCT FROM 2;
COMMIT;

-- 注意：テーブル定義確認クエリは common_table_definitions.sql を参照してください

