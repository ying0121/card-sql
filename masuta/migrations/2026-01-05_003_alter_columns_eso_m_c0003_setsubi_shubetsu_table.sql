...
-- 2026-01-05_003_alter_columns_eso_m_c0003_setsubi_shubetsu.sql
-- 目的：設備種別マスタテーブルのカラムを変更
-- 作成日: 2026-01-05
-- 作成者: 浦大輔
-- 対象DB：PostgreSQL
...
BEGIN
    IF EXISTS (
        ...
        WHERE table_name = 'eso_m_c0003_setsubi_shubetsu' 
...
-- shubetsu_nm を VARCHAR(32) から CHARACTER VARYING(32) に変更し、NOT NULL を設定
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'eso_m_c0003_setsubi_shubetsu' 
        AND column_name = 'shubetsu_nm'
    ) THEN
        ALTER TABLE eso_m_c0003_setsubi_shubetsu
            ALTER COLUMN shubetsu_nm TYPE CHARACTER VARYING(32),
            ALTER COLUMN shubetsu_nm SET NOT NULL;
    END IF;
END $$;

-- table_nm を VARCHAR(64) から CHARACTER VARYING(64) に変更
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'eso_m_c0003_setsubi_shubetsu' 
        AND column_name = 'table_nm'
    ) THEN
        ALTER TABLE eso_m_c0003_setsubi_shubetsu
            ALTER COLUMN table_nm TYPE CHARACTER VARYING(64);
    END IF;
END $$;
