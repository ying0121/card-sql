...
-- 2026-01-07_003_alter_columns_eso_m_c0009_setsubi_category.sql
...
BEGIN
    IF EXISTS (
        ...
        WHERE table_name = 'eso_m_c0009_setsubi_category' 
        AND column_name = 'UPDATE_DT'
    ) THEN
        ALTER TABLE eso_m_c0009_setsubi_category
...
-- shubetsu_cd (CHAR(4)) → character(4), Not Null: Yes (PK)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
