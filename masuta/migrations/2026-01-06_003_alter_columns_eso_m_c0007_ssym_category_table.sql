...
-- 2026-01-06_003_alter_columns_eso_m_c0007_ssym_category.sql
...
BEGIN
    IF EXISTS (
        ...
        WHERE table_name = 'eso_m_c0007_ssym_category' 
        AND column_name = 'UPDATE_DT'
    ) THEN
        ALTER TABLE eso_m_c0007_ssym_category
...
BEGIN
    IF EXISTS (
        ...
        ALTER TABLE eso_m_c0007_ssym_category
            RENAME COLUMN CATEGORY_CD TO category1_cd;
    END IF;
END $$;
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'eso_m_c0007_ssym_category' 
        AND column_name = 'category1_cd'
    ) THEN
        ALTER TABLE eso_m_c0007_ssym_category
            ALTER COLUMN category1_cd SET NOT NULL;
    END IF;
END $$;
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'eso_m_c0007_ssym_category' 
        AND column_name = 'ZUMEN_CD'
    ) THEN
        ALTER TABLE eso_m_c0007_ssym_category
            RENAME COLUMN ZUMEN_CD TO zumen_kind;
    END IF;
END $$;
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'eso_m_c0007_ssym_category' 
        AND column_name = 'zumen_kind'
    ) THEN
        ALTER TABLE eso_m_c0007_ssym_category
            ALTER COLUMN zumen_kind SET NOT NULL;
    END IF;
END $$;
